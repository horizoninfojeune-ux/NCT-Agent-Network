...
async function loadDashboard() {
  ...
  try {
    ...
    $("#profile").innerHTML = `
      ...
      <p>${escapeHtml(me.agent.phone || "Téléphone non renseigné")}</p>
      <p>
        ${escapeHtml(me.agent.island || "Île non renseignée")}
        ·
        ${escapeHtml(me.agent.locality || "Localité non renseignée")}
      </p>
    `;
...
async function loadNetwork() {
  ...
  $("#totalAgents").textContent = data.totalAgents;
      ...
      ? data.agents.map(a => `
        <div class="agent">
          ...
          <small>
            ${escapeHtml(a.role)}
            ·
            ${escapeHtml(a.island || "Zone non renseignée")}
            ·
            ${escapeHtml(a.locality || "Localité non renseignée")}
          </small>
        </div>
      `).join("")
...
$("#registerForm").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    show("#registerMessage", err.message);
  }
});
